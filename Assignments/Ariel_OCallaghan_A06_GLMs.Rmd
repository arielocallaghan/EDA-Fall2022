...
> Answer:
The R-squared value is 0.67 percent which means that 67% of the data is repersented by 34754 degrees of freedom. 
Also you have a negative correlcation of - 0.81 between temperature and depth. 
...
#fun error - delete
corrplot(NTL.chem.wrangled, method = "ellipse")
corrplot.mixed(NTL.chem.wrangled, upper = "ellipse")

#10
tempregression <-lm(data = NTL.chem.wrangled, temperature_C ~ year4  + daynum + depth)
summary(tempregression)

#tempplot <-ggplot(NTL.chem.wrangled,aes(x= year4, y= temperature_C))+
#  geom_point() 
#print(tempplot)
...
> Answer: 
The R-squared value increased from .67 when just using depth to 0.74 when adding in year4, daynum. A regression run with out depth returns an R squared value of 0.002363. The AIC values were all close together 26066 - 39189 and were kept in for further analysis. 
...
bartlett.test(NTL.chem.wrangled$lakename ~NTL.chem.wrangled$temperature_C)

anova.lakes <- aov(data = NTL.chem.wrangled, temperature_C ~ lakename)
summary(anova.lakes)

anova.lakes.raw <- aov(data = NTL.chem.raw, temperature_C ~ lakename)
summary(anova.lakes.raw)

anova.lakes.lm <- lm(data = NTL.chem.wrangled, temperature_C ~ lakename)
summary(anova.lakes.lm)

anova.lakes.lm.raw <- lm(data = NTL.chem.raw, temperature_C ~ lakename)
summary(anova.lakes.lm.raw)
```
