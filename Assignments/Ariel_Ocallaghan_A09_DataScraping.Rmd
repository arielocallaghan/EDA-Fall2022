...
#4
# Order of months fro scraped data. Month <- c(1,5,9,2,6,10,3,7,11,4,8,12)
...
#6.
scrape.it.2 <- function(pswid, year){

  
the.website.2 <-read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=', pswid, '/', year))
...
df_withdrawals.3<-data.frame("Month" = c(1,5,9,2,6,10,3,7,11,4,8,12), 
                          "Year"= rep(year,12),
                          "Max_Withdrawls_mgd"= as.numeric(max.withdrawals.mgd))

df_withdrawals.3 <- df_withdrawals.3 %>% 
  mutate(Water_system_name = !!water.system.name,
         PSWID = !!pswid,
         Ownership = !!ownership,
         Date = my(paste(Month,"-",Year)))

return(df_withdrawals.3)

df_scrape.3 <- scrape.it('03-32-010', 2015)
view(df_scrape.3)
