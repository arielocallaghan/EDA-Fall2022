...
the_website <- read_html(paste0("https://www.ncwater.org/WUDC/app/LWSP/search.php?pwsid=", the_pwsid,'&year=', the_year ))
...
ggplot(df_durham_2015 ,aes(x=Date,y=max.withdrawals.mgd)) +
  geom_line()+
  labs(title = paste("2015 Max Daily Water Withdrawal"), X = "Date", y = " Max Daily Withdrawals (mgd)")
```
...
ggplot(durham_asheville,aes(x=Date,y=max.withdrawals.mgd, color=water.system.name)) +
  geom_line() +
...
ggplot(Ash_Together,aes(x=Date,y=max.withdrawals.mgd, color = Water_System)) +
geom_line() +
