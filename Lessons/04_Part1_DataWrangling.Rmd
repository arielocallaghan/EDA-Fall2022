---
title: "4: Part 1 - Data Wrangling -Ariel"
author: Environmental Data Analytics | John Fay and Luana Lima | Developed by Kateri
  Salk
date: "Fall 2022"
output:
...
#check to see if dates ran correctly 
```
...
`==` compare value to a specific value, if statements
`!=`compare to see if values are different
`<`
`<=`
`>`
`>=`
...
# matrix filtering # filtering only rows wth 0 as column depth, filtering by rows. 
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering filter (dataset, what you are trying to filter)
#0.25 is also surface information so used seperate filter
...
# filter only uses rows
# filter function select paul lake | OR function, peter lake
# != exclude lakes from final data set
# %in% lakename %in% (vector with all names you are looking at) same as line 121 results
...
# look at excel sheet on daynum to corresponding date when using filter function
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) 
# use $ becuase greater then 151 and less then 305. the , also is the same thing and should get similar value
...
# last day of octover included and firest date of other month with >=
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304))
# add a vector with datenum values that are accecptable with our operations
...
# if need decending order need to specify in dataset 
#arrange (datset, colum to arrange)
# Exercise: 
...
# respect to columns. Filter and arrange is for rows. 
# dataframe<- select (data, lakename, column that you want) 
#columns you want - sampledate - need date information, depth, and temperature
# : column symbol with all columbs range inbeween both sides of the :
```
### Mutate
...
```{r}
#add new functions that are functions off of existing columns
# adding a new column to an existing data set (temperate = (function/ operation of change))
...
#funtion month from luberdate - extracts only month from dataset
# month= month(sampledate) creates a new column month and takes month data out of that column
...
#selecting columns and re ordering them, not changing data
...
# create a interval object that has a sepcificed start and end date
# can also use first and last part of luberdate, using column you need - same result as interval that willl work for anysize data set
...
```{r}
#can be replaced by the word then
#start with orgional data, filter by lake name - pipe does need name of dataset only conditions
#select same columns used befre
#muate also don't need origional data set, end each line with the piep operator
...
```{r}
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "./Data/Ariel_processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv")
```
