...
#url file robots.txt to determine if they have an legal footing for scraping 
```
...
```{r scrape.some.data}
#.table tr:nth-child(1) td:nth-child(2) copy from selector gadget tool 
...
#AO test tr:nth-child(2) th+ .left:nth-child(2)
...
#AO test .table:nth-child(7) td:nth-child(7) , .table:nth-child(7) th:nth-child(3) , .table:nth-child(7) th:nth-child(7) , .table:nth-child(7) td:nth-child(3)
...
#Create a dataframe of withdrawals # rep comand focus on sequence of numbers - months no scraped from data 
...
#Conflate the returned dataframes into a single dataframe - similar to r bind
...
## removed in lesson was part of plot above but didn't update correctly 
  #labs(title = paste("2020 Water usage data for",the_registrant),
      # subtitle = the_facility_name,
     #  y="Withdrawal (mgd)",
      # x="Date")
...
#"Map" the "scrape.it" function to retrieve data for all these - map 2 for more then one item. Itterates through the lists in parallel 
