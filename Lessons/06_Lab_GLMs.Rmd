---
title: "6: Part 1 - Generalized Linear Models-Ariel"
...
O3.density <-
  ...
  geom_density(alpha=0.5)+ scale_fill_manual(values= c("darkgreen", "darkblue"))+ labs(x="Ozone AQI values", y= "Density", fill = "")
print(O3.density)
...
O3.boxplot<-
 ggplot(EPAair, aes(x= as.factor(Year), y=Ozone))+
  geom_boxplot()+
  stat_summary(geom = "text", fun=max, vjust =-1, size = 4, label= c("b", "a"))+ labs(x= "", y= "Ozone AQI values") +
  ylim(0,150)
print(O3.boxplot)
...
O3.violin<-
  ggplot(EPAair, aes(x= as.factor(Year), y=Ozone))+
  geom_violin(draw_quantiltes= 0.5)+
  stat_summary(geom = "text", fun=max, vjust =-1, size = 4, label= c("b", "a"))+ labs(x= "", y= "Ozone AQI values") +
  ylim(0,150)
print(O3.violin)
```
...
#Exercise 2: Run a linear regression PM2.5 by Ozone. Find the p-value and R-squared value. Predict pm2.5 so that will be y because we are prediction from ozone. 

PM2.5byOzone.regression <- lm(data = EPA)
...
#Exercise 5: Choose a model by AIC in a Stepwise Algorithm. Do the results from AIC match the variables you selected on Exercise 5? dakldfd
...
#order of groups by plot id, order them by how they appear in the treat groups. Make plots look better. 

#plot from exercise 6 above 
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) + order_by($nlcdClass)+
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("abc", "ab", "abcd", "bcd", "bcd", "a", 
