...
### Themes
Often, we will want to change multiple visual aspects of a plot. Ggplot comes with pre-built themes that will adjust components of plots if you call that theme. where legends should go set these for graphs
...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #default ggi plot theme
print(O3plot1)

#theme bw - is dark on white theme- background white
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #background is white - grey axis lines
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #no gridlines x and y axis lines
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #define theme - call you theme - 
  theme(axis.text = element_text(color = "black"), #change axis text and legend position top
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme #setting theme using plot environment from code above 
print(O3plot5)  
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #lyt speciffy type of line - dashed line
  geom_hline(yintercept = 100, lty = 2) + #another horizontal line at point 100
  geom_point(alpha = 0.5, size = 1.5) + #add points representing data set - alpha determine transparacty and size - not changing shape. 
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # hjust = 1 means left align - margin you want to consider - write out where text is - label is what text you want printed out. font face- text = bold
  geom_text(x = as.Date("2020-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + #moderate now changing where on y axis it shows up
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + #
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")),  #change scale on x asis 
    date_breaks = "2 months", date_labels = "%b %y") + #month labels number of didgets that appear
  ylab(expression("O"[3]* " AQI Value")) +
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #so text on axis does not oerlap
print(O3plot7)  
...
# looks at color maps and different shades 
...
display.brewer.all(n = 9) #display all colors 
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + #add some transparency to the points 
...
#plot grid allows you to have multiple plots in same window. specify how many rows with how many plots. 2 plots 
#rel_height adjustes space of plots have in window- one plot may need more space 
```
